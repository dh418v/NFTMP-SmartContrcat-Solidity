...
async function main() {
  ...
  console.log(
    `USDC contract is deployed. Token address: ${instanceUSDC.target}`
  );

  const USDC_Address = await instanceUSDC.getAddress();
  const developmentTeam: string = "your wallet address";
  ...
  const instanceFactory = await ethers.deployContract("Factory", [
    Group_Address,
    Content_Address,
    Marketplace_Address,
    developmentTeam,
